...
import { EnhancementsListItem } from '../EnhancementsListItem';

export interface EnhancementsListProps {
  enhancements: IEnhancementListItem[];
  total: number;
  page: number;
  isLoading: boolean;
  onPageChange: (newPage: number) => void;
  onView: (item: IEnhancementListItem) => void;
  onEdit: (item: IEnhancementListItem) => void;
  onDelete: (item: IEnhancementListItem) => void;
}
          ...
          {!isLoading && enhancements.length === 0 && (
            <TableRow>
              <TableCell colSpan={3} sx={{ borderColor: APP_COLORS.gold }}>
                <DefaultText>Nenhum aprimoramento encontrado.</DefaultText>
              </TableCell>
            </TableRow>
          )}

          {enhancements.map((item) => (
            <EnhancementsListItem
              key={item.id}
              enhancement={item}
              onView={onView}
              onEdit={onEdit}
