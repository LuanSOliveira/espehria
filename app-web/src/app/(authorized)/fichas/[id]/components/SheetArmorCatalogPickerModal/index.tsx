...
import { ArmorsFilterSection } from '@/app/(authorized)/armaduras/components/ArmorsFilterSection';

export interface SheetArmorCatalogPickerModalProps {
  open: boolean;
  onClose: () => void;
  onSelect: (armor: IArmor) => void;
}

/**
 * Modal de seleção de item existente do catálogo de Armaduras — mesmo
 * espírito de `SheetWeaponCatalogPickerModal`.
 */
export const SheetArmorCatalogPickerModal = ({
  open,
  onClose,
  onSelect,
}: SheetArmorCatalogPickerModalProps) => {
  ...
  const { data, isLoading } = useGetEntityList<IArmorListItem, IArmorListFilters>({
    url: '/armors',
    filters,
    enabled: open,
  });

  const { data: armorDetail } = useGetEntityById<IArmor>({
    url: `/armors/${selectedId}`,
    enabled: !!selectedId,
  });

  useEffect(() => {
    if (selectedId && armorDetail) {
      onSelect(armorDetail);
      setSelectedId(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- só reage à chegada do detalhe após clique em "Selecionar"
  }, [selectedId, armorDetail]);
    ...
    >
      <Card sizeClassName="w-[min(1152px,92vw)]">
        ...
        <Title component="h2" sx={{ marginBottom: '12px' }}>
          Selecionar Armadura do Catálogo
        </Title>
            ...
            <TableBody>
              ...
              {(data?.data ?? []).map((armor) => (
                <TableRow key={armor.id}>
                  ...
                  <TableCell align="right" sx={{ borderColor: APP_COLORS.gold }}>
                    <MuiIconButton
                      aria-label={`Selecionar ${armor.name}`}
                      onClick={() => setSelectedId(armor.id)}
