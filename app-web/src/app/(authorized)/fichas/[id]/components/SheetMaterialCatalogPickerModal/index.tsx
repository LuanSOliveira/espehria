...
import { MaterialsFilterSection } from '@/app/(authorized)/materiais/components/MaterialsFilterSection';

export interface SheetMaterialCatalogPickerModalProps {
  open: boolean;
  onClose: () => void;
...
/**
 * Modal de seleção de item existente do catálogo de Materiais — mesmo
 * espírito de `SheetWeaponCatalogPickerModal`.
 */
export const SheetMaterialCatalogPickerModal = ({
  open,
  onClose,
  onSelect,
}: SheetMaterialCatalogPickerModalProps) => {
  ...
  const { data, isLoading } = useGetEntityList<IMaterialListItem, IMaterialListFilters>({
    url: '/materials',
    filters,
    enabled: open,
  });

  const { data: materialDetail } = useGetEntityById<IMaterial>({
    url: `/materials/${selectedId}`,
    enabled: !!selectedId,
  });

  useEffect(() => {
    if (selectedId && materialDetail) {
      onSelect(materialDetail);
      setSelectedId(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- só reage à chegada do detalhe após clique em "Selecionar"
  }, [selectedId, materialDetail]);
    ...
    >
      <Card sizeClassName="w-[min(1152px,92vw)]">
        ...
        <Title component="h2" sx={{ marginBottom: '12px' }}>
          Selecionar Material do Catálogo
        </Title>
            ...
            <TableBody>
              ...
              {(data?.data ?? []).map((material) => (
                <TableRow key={material.id}>
                  <TableCell sx={{ borderColor: APP_COLORS.gold }}>
                    <ImageAvatarPreview imageUrl={material.referenceImage} alt={material.name} />
                  </TableCell>
                  <TableCell sx={{ borderColor: APP_COLORS.gold }}>
                    <DefaultText>{material.name}</DefaultText>
                  </TableCell>
                  <TableCell sx={{ borderColor: APP_COLORS.gold }}>
