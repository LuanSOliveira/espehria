...
import { ShieldsFilterSection } from '@/app/(authorized)/escudos/components/ShieldsFilterSection';

export interface SheetShieldCatalogPickerModalProps {
  open: boolean;
  onClose: () => void;
  onSelect: (shield: IShield) => void;
}

/**
 * Modal de seleção de item existente do catálogo de Escudos — mesmo
 * espírito de `SheetWeaponCatalogPickerModal`.
 */
export const SheetShieldCatalogPickerModal = ({
  open,
  onClose,
  onSelect,
}: SheetShieldCatalogPickerModalProps) => {
  ...
  const { data, isLoading } = useGetEntityList<IShieldListItem, IShieldListFilters>({
    url: '/shields',
    filters,
    enabled: open,
  });

  const { data: shieldDetail } = useGetEntityById<IShield>({
    url: `/shields/${selectedId}`,
    enabled: !!selectedId,
  });

  useEffect(() => {
    if (selectedId && shieldDetail) {
      onSelect(shieldDetail);
      setSelectedId(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- só reage à chegada do detalhe após clique em "Selecionar"
  }, [selectedId, shieldDetail]);
        ...
        <TableContainer className="mt-6">
          <Table>
            ...
            <TableBody>
              ...
              {(data?.data ?? []).map((shield) => (
                <TableRow key={shield.id}>
                  <TableCell sx={{ borderColor: APP_COLORS.gold }}>
                    <ImageAvatarPreview imageUrl={shield.referenceImage} alt={shield.name} />
                  </TableCell>
                  <TableCell sx={{ borderColor: APP_COLORS.gold }}>
                  ...
                  <TableCell align="right" sx={{ borderColor: APP_COLORS.gold }}>
                    <MuiIconButton
                      aria-label={`Selecionar ${shield.name}`}
                      onClick={() => setSelectedId(shield.id)}
