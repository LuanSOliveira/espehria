...
export interface MaterialCustomDataPayload {
  name: string;
...
export interface SheetMaterialStandaloneFormProps {
  onSubmit: (payload: MaterialCustomDataPayload) => void;
}

/**
 * Réplica de `MaterialCreateForm` sem persistência no catálogo — mesmo
 * espírito de `SheetWeaponStandaloneForm`.
 */
export const SheetMaterialStandaloneForm = ({
  onSubmit,
}: SheetMaterialStandaloneFormProps) => {
  const { tagOptions } = useTagOptionsQuery();

  const { data: currenciesData } = useCurrenciesQuery();
  const currencyOptions = currenciesData ?? [];

  const { control, handleSubmit } = useForm<MaterialFormData>({
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <FormTextInput
          id="sheet-material-form-reference-image"
          name="referenceImage"
          control={control}
        ...
        <FormAutocompleteInput<MaterialFormData, ICurrency>
          id="sheet-material-form-currency"
          name="currencyId"
          control={control}
          label="Moeda"
      ...
      <div className="grid grid-cols-1 gap-4">
        <FormRichTextInput
          id="sheet-material-form-description"
          name="description"
          control={control}
          label="Descrição"
      ...
      <FormRichTextInput
        id="sheet-material-form-private-information"
