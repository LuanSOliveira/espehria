...
export interface EquipmentViewProps {
  equipmentId: string;
  /**
   * Chamado quando o equipamento não é encontrado (404) — usado pelo
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row">
        {equipment.referenceImage ? (
          <>
            <button
              type="button"
              aria-label={`Ampliar imagem de ${equipment.name}`}
            ...
            <ImagePreviewDialog
              ...
              imageUrl={equipment.referenceImage}
              alt={equipment.name}
            />
          </>
        ) : (
      ...
      {!isGoogleUser && (
        ...
        >
          ...
          <div className="flex-1 px-3 py-3">
            <RichTextViewer
              value={equipment.privateInformation}
              emptyLabel={NOT_INFORMED}
            />
          </div>
        </div>
