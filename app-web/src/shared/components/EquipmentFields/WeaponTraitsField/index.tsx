...
import { useArmaTraitTypeId } from '@/hooks/Queries';
import { IEntityReference } from '@/shared/interfaces';
import { showToast } from '@/shared/util';

export interface WeaponTraitsFieldProps {
  value: IEntityReference[];
  onChange: (value: IEntityReference[]) => void;
}

export const WeaponTraitsField = ({ value, onChange }: WeaponTraitsFieldProps) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { armaTraitTypeId, isLoading } = useArmaTraitTypeId();
        ...
        tabs={[
          {
            ...
            entityType: 'trait',
            url: '/traits',
            extraFilters: { traitTypeId: armaTraitTypeId },
          },
        ]}
      />
    </div>
