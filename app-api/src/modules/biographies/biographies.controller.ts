...
export class BiographiesController {
  ...
  @ApiNotFoundResponse({
    description:
      'Biografia, uma ou mais tags/entidades referenciadas em Habilidades Adicionais, tipos/propriedades de Melhorias, propriedades/graduações de Proficiências, ou graduações de Saberes não encontrados',
  })
  @ApiConflictResponse({
    description:
      'Já existe uma biografia com este nome, ou violação de regra em Habilidades Adicionais (autorreferência ou duplicata), ou violação de regra em Melhorias (duplicidade de combinação Tipo×Propriedade na mesma lista ou incompatibilidade entre o Tipo e a Propriedade selecionados), ou violação de regra em Proficiências (duplicidade de propriedade na mesma lista), ou violação de regra em Saberes (título duplicado na mesma lista)',
  ...
  @ApiBadRequestResponse({
    description:
      'ID em formato inválido, formato inválido de entityType/id em Habilidades Adicionais, formato inválido de value/type/property em Melhorias (value deve ser inteiro ≥ 1, type/property devem ser UUIDs válidas), formato inválido de property/gradation em Proficiências (devem ser UUIDs válidas), ou formato inválido de title/gradation em Saberes (title deve ser string não vazia, gradation deve ser UUID válido)',
  })
  async update(
    @Param('id', ParseUUIDPipe) id: string,
