  ...
  async findAllPaginated(
    ...
    if (hasTagFilter) {
      ...
      queryBuilder
        .innerJoin(
          'biography_tags',
          'biography_tag_filter',
          'biography_tag_filter.biography_id = biography.id AND biography_tag_filter.tag_id IN (:...tagIds)',
          { tagIds: uniqueTagIds },
        )
        .groupBy('biography.id')
        .having('COUNT(DISTINCT biography_tag_filter.tag_id) = :tagCount', {
    ...
    // (uma por biografia). Sem filtro de tags, `getCount()` é suficiente e
    // evita trazer todos os ids para a aplicação só para contá-los.
    const total = hasTagFilter
      ? (await queryBuilder.clone().select('biography.id').getRawMany()).length
