...
export class RacesService {
  ...
  async findById(id: string): Promise<RaceWithReferences | null> {
    ...
    const knowledges = await this.knowledgesService.loadItemsFor(
      KnowledgeOwnerType.RACE,
      id,
    );

    return { race, improvements, flaws, proficiencies, knowledges };
  }
  ...
  async findAllPaginated(query: FindRacesQueryDto): Promise<PaginatedRaces> {
    ...
    if (hasTagFilter) {
      ...
      queryBuilder
        ...
        .groupBy('race.id')
        .having('COUNT(DISTINCT race_tag_filter.tag_id) = :tagCount', {
    ...
    // (uma por raça). Sem filtro de tags, `getCount()` é suficiente e evita
    // trazer todos os ids para a aplicação só para contá-los.
    const total = hasTagFilter
      ? (await queryBuilder.clone().select('race.id').getRawMany()).length
