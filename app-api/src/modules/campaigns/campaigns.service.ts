...
export class CampaignsService {
  ...
  async findAllPaginated(
    ...
    if (hasTagFilter) {
      ...
      queryBuilder
        .innerJoin(
          'campaign_tags',
          'campaign_tag_filter',
          'campaign_tag_filter.campaign_id = campaign.id AND campaign_tag_filter.tag_id IN (:...tagIds)',
          { tagIds: uniqueTagIds },
        )
        .groupBy('campaign.id')
        .having('COUNT(DISTINCT campaign_tag_filter.tag_id) = :tagCount', {
    ...
    // `createdBy`), contando as linhas resultantes (uma por campanha). Sem
    // filtro de tags, `getCount()` é suficiente e evita trazer todos os ids
    // para a aplicação só para contá-los.
    const total = hasTagFilter
      ? (await queryBuilder.clone().select('campaign.id').getRawMany()).length
