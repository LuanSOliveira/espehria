...
@Check(
  ...
  'num_nonnulls(owner_training_id, owner_talent_id, owner_technique_id, owner_spell_id, owner_characteristic_id, owner_biography_id) = 1',
)
@Check(
  'CK_entity_links_target_exclusive',
  'num_nonnulls(target_training_id, target_talent_id, target_technique_id, target_spell_id, target_characteristic_id, target_biography_id) = 1',
)
@Unique([
  'linkType',
  'ownerTraining',
...
export class EntityLink extends BaseEntity {
  ...
  @ManyToOne(() => Biography, { nullable: true, onDelete: 'CASCADE' })
  @JoinColumn({ name: 'target_biography_id' })
  targetBiography!: Biography | null;
}
