...
export class TraitsService {
  ...
  async findAllPaginated(query: FindTraitsQueryDto): Promise<PaginatedTraits> {
    ...
    if (hasTagFilter) {
      ...
      queryBuilder
        .innerJoin(
          'trait_tags',
          'trait_tag_filter',
          'trait_tag_filter.trait_id = trait.id AND trait_tag_filter.tag_id IN (:...tagIds)',
          { tagIds: uniqueTagIds },
        )
        .groupBy('trait.id')
        .having('COUNT(DISTINCT trait_tag_filter.tag_id) = :tagCount', {
    ...
    // (um por traço). Sem filtro de tags, `getCount()` é suficiente e evita
    // trazer todos os ids para a aplicação só para contá-los.
    const total = hasTagFilter
      ? (await queryBuilder.clone().select('trait.id').getRawMany()).length
