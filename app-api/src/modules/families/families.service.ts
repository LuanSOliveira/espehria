...
export class FamiliesService {
  ...
  async findAllPaginated(
    ...
    if (hasTagFilter) {
      ...
      queryBuilder
        .innerJoin(
          'family_tags',
          'family_tag_filter',
          'family_tag_filter.family_id = family.id AND family_tag_filter.tag_id IN (:...tagIds)',
          { tagIds: uniqueTagIds },
        )
        .groupBy('family.id')
        .having('COUNT(DISTINCT family_tag_filter.tag_id) = :tagCount', {
    ...
    // (uma por família). Sem filtro de tags, `getCount()` é suficiente e
    // evita trazer todos os ids para a aplicação só para contá-los.
    const total = hasTagFilter
      ? (await queryBuilder.clone().select('family.id').getRawMany()).length
